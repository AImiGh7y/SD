package edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.gui;
...
import edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.engine.Pathfinding;

/**
 * Depending on if a unit is selected by the current player and the edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.units state, it will draw the movement range (based on path finding outcome).
 ...
 * It will also draw the attackable locations depending on the edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.units max and min attack ranges.
 * @author SergeDavid
 * @version 0.3
 */
public class Ranges {
	public static void Draw(Graphics2D gg, int resize) {
		if (Game.player.get(Game.btl.currentplayer).unitselected) {
			...
			edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.units.Base unit = Game.units.get(Game.player.get(Game.btl.currentplayer).selectedunit);
	...
	private static void Moving(Graphics2D gg, edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.units.Base unit, int size, int resize) {
