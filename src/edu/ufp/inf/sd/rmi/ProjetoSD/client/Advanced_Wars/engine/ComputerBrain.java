...
public class ComputerBrain {
	...
	public void ThinkDamnYou(edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.players.Base ply) {
		...
		if (!DoneUnits) {
			edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.units.Base unit = Game.units.get(UnitCount);
			UnitCount++;
			...
			edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.buildings.Base bld = Game.builds.get(CityCount);
			CityCount++;
	...
	private void HandleBuilding(edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.buildings.Base bld) {
		if (!bld.Menu.equals("")) {
			if (!bld.Locked) {
	...
	/**Looks for any enemies in range at the edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.units current location by attacking each location in the attackable locations.*/
	private boolean WannaHug(edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.units.Base unit) {
	...
	private boolean BuildingInRange(edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.units.Base unit) {
