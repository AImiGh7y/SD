...
public class Battle {
	/**A count of all the edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.players in the game, used before Game.player is populated so this is required.*/
	...
	public void NewGame(String mapname, String uid, ObserverImpl observer, SubjectRI subjectRI, int playerId) {
		...
		Game.player = new ArrayList<edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.players.Base>();
		Game.builds = new ArrayList<edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.buildings.Base>();
	...
	public void MaxUsers(int max) {
		//Setup for max edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.players
		if (max<2) {totalplayers = 2;}
		else if (max>12) {totalplayers = 12;}
		else {totalplayers = max;}
		//HACK: Change when I add more images to support more edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.players.
	...
	public void CaptureCapital(int x, int y) {
		...
		for (int i = 0; i < Game.builds.size(); i++) {
			if (Game.builds.get(i).x == x && Game.builds.get(i).y == y) {
				...
				if (playersleft<=1) {
					GameOver = true;
					new edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.menus.EndBattle();
				}
				break;
			}
		}
		...
		//TODO: Change all edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.buildings to be owned by the player.
	}
