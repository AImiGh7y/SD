...
/**
 * When SaveGame() is called upon it grabs a screen shot of the current battle (current_player,days) and the important variables from the player, edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.buildings, and unit lists.
 * It currently saves them in a property file, with each saved variable having their own line (it gets very long fast).
 * When LoadGame() is called, it will create a new battle, change the current day and player, and then loads the edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.players / edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.units / cities from here.
...
public class Save {
	...
	public void SaveGame() {
		...
		try {
		    ...
		    out.write("# A testing save file format for my advanced wars project, using property file as it is a list of properties...\n" +
		    		  "# When loading map, skip the unit stage (will load edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.units from here, maybe edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.buildings too with health)" +
	...
	private String SavePlayerData() {
		...
		for (int i = 0; i < Game.btl.totalplayers; i++) {
	    	edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.players.Base ply = Game.player.get(i);
	    	plyrdata+="Player" + i + "_Type = " + ply.name + "\n" +//This is the character (commander) that said player is using. (Using name to find them)
	    			  ...
	    			  "Player" + i + "_Kills = " + ply.kills + "\n" +//How many edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.units they've killed
	    			  "Player" + i + "_Loses = " + ply.loses + "\n" +//How many edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.units they've lost
	...
	private String SaveCityData() {
		...
		for (int i = 0; i < Game.builds.size(); i++) {
	    	edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.buildings.Base bld = Game.builds.get(i);
	...
	private String SaveUnitData() {
	    ...
	    for (int i = 0; i < Game.units.size(); i++) {
	    	edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.units.Base unit = Game.units.get(i);
	...
	public void LoadGame() {
		try {
			...
			Game.btl.NewGame(configFile.getProperty("Map"), null, null, null, 0);
			...
			Game.units = new ArrayList<edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.units.Base>();
			...
			//Changes the edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.gui to match the appropriate bla bla bla
