...
public class Map {
	...
	public edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.terrain.Base[][] map;
	public List<edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.terrain.Base> tiles = new ArrayList<edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.terrain.Base>();
	public MapParser parse = new MapParser();
	
	public Map() {
		LoadTiles();
		map = new edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.terrain.Base[height][width];
	}
	...
	public void MapSetup(int width, int height) {
		...
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
				map[y][x] = new edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.terrain.Dirt();
			}
		}
	}
	...
	private void ChangeTiles(edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.terrain.Base type, int x, int y){
	...
	public edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.terrain.Base getTile(int i) {
		switch(i) {
		case 0:return new edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.terrain.Dirt();
		case 1:return new edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.terrain.Forest();
		case 2:return new edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.terrain.Mountain();
		case 3:return new edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.terrain.Water();
		case 4:return new edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.terrain.City();
		case 5:return new edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.terrain.Road();
		default:return null;
		}
	}
	...
	public void ResizeMap(int neww, int newh, int oldw, int oldh) {
		...
		width = neww;
		height = newh;
		edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.terrain.Base[][] newmap = new edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.terrain.Base[height][width];
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {//TODO: Fix this as it seems to not be exactly correct.
				if (oldh < y || oldw < x) {
					...
					newmap[y][x] = new edu.ufp.inf.sd.rmi.ProjetoSD.client.Advanced_Wars.terrain.Dirt();
				}
			}
		}
		map = newmap;
