...
public class Base {
	...
	public boolean MoveAndShoot = true;//Allows edu.ufp.inf.sd.rabbitmqservices._ProjetoSD.client.Advanced_Wars.units to move then shoot, or make them stay in the same location in order to attack.
	public int MaxAtkRange = 1;//How many squares away from the unit can it attack. (1 being default, 0 being none)
	public int MinAtkRange = 1;//This is used for ranged edu.ufp.inf.sd.rabbitmqservices._ProjetoSD.client.Advanced_Wars.units such as artillery.
	...
	public boolean PathCheck(int destx, int desty) {
		...
		for (edu.ufp.inf.sd.rabbitmqservices._ProjetoSD.client.Advanced_Wars.units.Base unit : Game.units) {//Allows edu.ufp.inf.sd.rabbitmqservices._ProjetoSD.client.Advanced_Wars.units in the same team to walk past each other.
	 ...
	 * @param returnfire = Allows edu.ufp.inf.sd.rabbitmqservices._ProjetoSD.client.Advanced_Wars.units to shoot back and not cause a never ending loop until someone dies.
	...
	public boolean attack(int destx, int desty, boolean returnfire) {
		...
		edu.ufp.inf.sd.rabbitmqservices._ProjetoSD.client.Advanced_Wars.units.Base target = FindTarget(destx, desty, true, false);
		if (target!=null) {
	...
	private void capture(int destx,int desty) {
		...
		if (destx==x&&desty==y) {
			if (bld!=-1) {
				edu.ufp.inf.sd.rabbitmqservices._ProjetoSD.client.Advanced_Wars.buildings.Base city = Game.builds.get(bld);
	...
	private edu.ufp.inf.sd.rabbitmqservices._ProjetoSD.client.Advanced_Wars.units.Base FindTarget(int destx, int desty, boolean hostilefire, boolean friendlyfire) {
		for (edu.ufp.inf.sd.rabbitmqservices._ProjetoSD.client.Advanced_Wars.units.Base unit : Game.units) {
