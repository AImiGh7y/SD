 ...
 * Include if(showing) {edu.ufp.inf.sd.rabbitmqservices._ProjetoSD.client.Advanced_Wars.gui.add(this);} when changing edu.ufp.inf.sd.rabbitmqservices._ProjetoSD.client.Advanced_Wars.gui elements (As I remove all elements when changing between game states such as in game / main menu / editor)
 * @author SergeDavid
 * @version 0.2
 */
public class ErrorHandler extends JPanel {
	...
	public void ShowError(String message) {
		...
		ResetSize();
		edu.ufp.inf.sd.rabbitmqservices._ProjetoSD.client.Advanced_Wars.engine.Game.gui.add(this);
		showing=true;
		showtime = 5;
	}
	/**This removes the error message from the edu.ufp.inf.sd.rabbitmqservices._ProjetoSD.client.Advanced_Wars.gui and sets the boolean to false.*/
	private void HideError() {
		if (hiddenlist.isEmpty()) {
			edu.ufp.inf.sd.rabbitmqservices._ProjetoSD.client.Advanced_Wars.engine.Game.gui.remove(this);
			showing=false;	
		}
		else {
	...
	private void ResetSize() {
		Insets insets = edu.ufp.inf.sd.rabbitmqservices._ProjetoSD.client.Advanced_Wars.engine.Game.gui.getInsets();
