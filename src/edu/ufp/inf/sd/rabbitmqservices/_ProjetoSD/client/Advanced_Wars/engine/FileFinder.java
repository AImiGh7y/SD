...
public class FileFinder {
	final String mappath = "C:\\Users\\Nuno\\SD\\maps";
	...
	public DefaultListModel GrabMaps() {
		...
		if (folder.mkdir()) {
			edu.ufp.inf.sd.rabbitmqservices._ProjetoSD.client.Advanced_Wars.engine.Game.error.ShowError("The " + mappath + " folder was created.");
	...
	public DefaultListModel GrabPacks() {
		...
		if (folder.mkdir()) {
            edu.ufp.inf.sd.rabbitmqservices._ProjetoSD.client.Advanced_Wars.engine.Game.error.ShowError("The " + packpath + " folder was created.");}
		File[] filelist = folder.listFiles();
		DefaultListModel ListModel = new DefaultListModel();
		for (int i = 0; i < filelist.length; i++) {
			if (filelist[i].isFile()) {
	...
	public DefaultListModel GrabMods() {
		...
		if (folder.mkdir()) {
            Game.error.ShowError("The " + modpath + " folder was created.");}
		File[] filelist = folder.listFiles();
		DefaultListModel ListModel = new DefaultListModel();
		for (int i = 0; i < filelist.length; i++) {
			if (filelist[i].isFile()) {
