...
public class Pathfinding {
	...
	private edu.ufp.inf.sd.rabbitmqservices._ProjetoSD.client.Advanced_Wars.units.Base unit;
	...
	public Vector<Point> FindPath(edu.ufp.inf.sd.rabbitmqservices._ProjetoSD.client.Advanced_Wars.units.Base unit, int range) {
		maphits = new int[edu.ufp.inf.sd.rabbitmqservices._ProjetoSD.client.Advanced_Wars.engine.Game.map.height][edu.ufp.inf.sd.rabbitmqservices._ProjetoSD.client.Advanced_Wars.engine.Game.map.width];
	...
	private void FindNodes() {
		...
		if (current.loc.x+1< edu.ufp.inf.sd.rabbitmqservices._ProjetoSD.client.Advanced_Wars.engine.Game.map.width&&unit.PathCheck(current.loc.x+1, current.loc.y)) {
			AddNode(current.loc.x+1,current.loc.y);
		}
		if (current.loc.y+1< edu.ufp.inf.sd.rabbitmqservices._ProjetoSD.client.Advanced_Wars.engine.Game.map.height) {
	...
	private void SwitchParent(PathNode node) {
		double cost = edu.ufp.inf.sd.rabbitmqservices._ProjetoSD.client.Advanced_Wars.engine.Game.map.map[node.loc.y][node.loc.x].speed() + current.cost;
		if (cost<node.cost) {
	...
	private double FindCost(int x, int y, PathNode parent) {
		double cost = edu.ufp.inf.sd.rabbitmqservices._ProjetoSD.client.Advanced_Wars.engine.Game.map.map[y][x].speed();
		if (parent!=null) {
			cost+=parent.cost;
		}
